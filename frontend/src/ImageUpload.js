...
const ImageUpload = ({onUploadSuccess}) => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      toast.success('File uploaded successfully');
      setrefresh(1);
      //refreshing the get-all ads component here
      onUploadSuccess();
    } catch (error) {
      console.error("Error uploading file:", error);
      alert("Error uploading file");
  ...
  return (
    <div className="">
      <h2>Upload Image</h2>
      <div>
        <input ref={fileInputRef} multiple={true} type="file" onChange={handleFileChange} />
      </div>
        ...
        <div className="">
          <div className="my-4">
            ...
            <div className="flex items-center justify-start gap-5">
              <input type="radio" name="closeBtnColor" value="0"  checked={closeBtnColor===0} onChange={(e) => setCloseBtnColor(0)} />
              <label className="">Light</label>
            </div>
            <div className="flex items-center justify-start gap-5">
              <input type="radio" name="closeBtnColor" value="0" checked={closeBtnColor===1} onChange={(e) => setCloseBtnColor(1)} />
